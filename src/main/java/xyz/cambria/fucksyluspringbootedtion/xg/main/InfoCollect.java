...
public class InfoCollect {
    ...
    public static boolean run(Properties pps , List<NameValuePair> list , HttpClientContext context , CloseableHttpClient httpClient) throws IOException, InterruptedException {
...
/*        list.add(new BasicNameValuePair("Province" , new String(pps.getProperty("IDNum").substring(0,2) + "0000")));
        ...
        list.add(new BasicNameValuePair("text_2" , ""));
        list.add(new BasicNameValuePair("text_3" , ""));
        list.add(new BasicNameValuePair("text_4" , ""));
        ...
        try {
            UrlEncodedFormEntity payload = new UrlEncodedFormEntity(list , "UTF-8");
...
//        System.out.println(EntityUtils.toString(response.getEntity()));
        ...
        if (respEntity.contains("过期")) {
            log.warn("Checocode Expired.");
            return false;
        }

        if (respEntity.contains("成功")) {
